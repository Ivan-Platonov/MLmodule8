...
while educational_grant == '':
  try:
    educational_grant = int(input('Введите стипендию: '))
    if educational_grant <= 0:
      error_message_1()
      educational_grant = ''
  except ValueError:
    error_message_1()
...
if expenses > educational_grant:
  for mounth in range(1, 11):
    delta += expenses - educational_grant
    print(f'{mounth}. месяц, траты {round(expenses,2)}, не хватает {round(delta,2)}')
    expenses += expenses * 0.03
  print()
  print(f'Нужно получить у родителей {round(delta,2)} рублей')
else:
  print()
  print('Помощь от родителей не требуется')